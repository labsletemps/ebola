...
function generateParagraphs(id,data){
    ...
    $(id).append('<div id="para' + i + '" class="col-md-12 para"><h3><b>'
            + d['titre'] + '</b></h3>'
            + d['text'] + '</div>');
    });
...
function generateBarChart(id,datain){
    ...
    bar.append("text")
        ...
        .text(function(d,i) { if(i%2===0){return "morts";}
                    else {
                        return "malades";
                    }
                }
...
function updateinfographic(temppara){
    if(currentpara!==temppara){
        ...
        $('#barcharttitle').html('<i class="fa fa-map-marker"></i> GRAPHIQUE - ' + data[temppara].date);
        if(!compact){
...
function resizedw(){
    if($(window).width()<268){compact = true;} else {compact = false;}
...
if($(window).width()<268){compact = true;}            
generateParagraphs('#text',data);
generateTimeline('#timeline',data);
generateBarChart('#barchart',data[0]);
generateMap('#map');
